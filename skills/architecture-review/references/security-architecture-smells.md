...
## Stale or Nonexistent API Documentation

Symptom: the API surface documented (OpenAPI/Swagger spec, published API reference, internal wiki) has drifted from what the code actually exposes — endpoints added without being documented, documented endpoints that no longer exist or behave differently, or no machine-readable spec at all for a non-trivial API surface. Risk: undocumented/stale-documented endpoints are exactly the "shadow API" surface OWASP API9:2023 (Improper Inventory Management) describes — they are the ones least likely to have had a deliberate security review, are easy for a reviewer (human or automated) to simply miss during `04_attack_surface_mapping`, and are a common place for deprecated-but-still-live endpoints with weaker controls than their replacement to linger unnoticed. See `knowledge/owasp/owasp-api-top10.md` API9:2023. Escalate from an attack-surface coverage gap to a dedicated `api-documentation-staleness` `ARCH-*` recommendation when the drift is systemic (multiple endpoints, no ownership process for keeping the spec current) rather than one or two isolated stale entries.
