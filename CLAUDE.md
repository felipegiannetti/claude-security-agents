...
Contains generated security reports -- but only in the context of this project's OWN development and testing (running the pipeline against this repo's own fixtures, or against a repo you own and are free to write into). When this plugin is deployed and used to review a THIRD-PARTY target repository, no file may be created inside that target at all -- see "Absolute Read-Only Policy" above, which explicitly forbids creating files inside the analyzed repository, full stop, with no "just the report" exception. In that deployed context, the final report is ALWAYS presented directly in the conversation AND saved as a file -- automatically, not only when asked -- but that file always goes OUTSIDE the analyzed repository, via `scripts/reporting/resolve_report_path.py` (default `~/SecurityReviews/<project>-<timestamp>/`), never into an `outputs/` folder inside the project being reviewed. A review must never fail or refuse to complete because the target project lacks an `outputs/` directory or any other structure from this project -- the target is never expected to look like this repository.
